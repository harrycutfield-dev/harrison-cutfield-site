...
- Visitors see the listings **frosted/blurred** with a form: Email (validated), Mobile (validated), property they own (optional), and buying timeframe (Now / 1 to 2 / 3 to 6 months / Longer).
...
## Your photos
Two slots are wired: your **portrait** (About section + team card) = `images/uploads/harrison-headshot.png`, and the **hero background** = `images/uploads/hero.jpg`. Add them either by dropping the files into `images/uploads/` with those exact names, or via the CMS (Site & Contact → portrait / hero image).
